  ...
  if (error || !weather) {
    return (
      <div className="bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-5 sm:p-6 text-white shadow-lg card-hover" onClick={handleClick}>
        ...
        <button onClick={handleRefresh} className="mt-2 text-xs text-blue-200 hover:text-white">点击重试</button>
      </div>
    );
  }
    ...
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between">
          <div>
            ...
            {current.feelsLike !== undefined && (
              <p className="text-blue-200 text-xs mt-0.5">体感 {Math.round(current.feelsLike)}°C</p>
            )}
            <p className="text-blue-200 text-[10px] mt-0.5">📡 {source || '香港天文台'}</p>
          </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          <p className="text-blue-200 text-xs">更新于 {fmtTime(current.updateTime)}</p>
