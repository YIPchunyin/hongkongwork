...
export default function MtrWidget() {
  ...
  if (error || !data) {
    return (
      <div className="bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl p-5 sm:p-6 text-white shadow-lg cursor-pointer hover:shadow-xl transition-all" onClick={handleClick}>
  ...
  // Get all trains sorted by ttnt
  let allTrains: { lineName: string; lineCode: string; dest: string; ttnt: number; plat: string; time: string; direction: string }[] = [];
  ...
  allTrains = allTrains.filter(t => t.ttnt <= 120); allTrains.sort((a, b) => a.ttnt - b.ttnt);
  const next3 = allTrains.slice(0, 3);

  return (
    <div
      className="bg-gradient-to-br from-orange-500 via-orange-600 to-red-600 rounded-2xl p-5 sm:p-6 text-white shadow-lg cursor-pointer hover:shadow-xl hover:-translate-y-0.5 transition-all group"
      onClick={handleClick}
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex gap-1">
          {station.lines.slice(0, 2).map((l) => (
            <span key={l.code} className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ backgroundColor: getLineColor(l.code) + '44', color: '#fff' }}>{l.code}</span>
          ))}
        </div>
      </div>

      {/* Next 3 trains with platform */}
      <div className="space-y-1.5">
        {next3.length > 0 ? next3.map((t, i) => (
          <div key={i} className="flex items-center justify-between bg-white/15 rounded-lg px-3 py-2 backdrop-blur">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-orange-200">🖥 {t.plat}号月台</span>
              <span className={'text-sm font-bold ' + (t.ttnt <= 1 ? 'text-yellow-300' : 'text-white')}>
                {t.ttnt}分
              </span>
            </div>
          </div>
      ...
      <div className="mt-2 flex items-center justify-between">
        <p className="text-orange-200 text-[10px]">🔄 每30秒刷新 · 更新 {fmtTime(sysTime)}</p>
        <span className="text-orange-200 text-xs group-hover:text-white transition-colors">全部班次 →</span>
      </div>
    </div>
  );
}
