...
export default function NotesPage(){
  ...
  const fn=useCallback(async()=>{
    ...
    try{
      ...
      const r=await fetch('/api/notes?'+p.toString());
      const j=await r.json();
      if(j.success){const d=j.data;sN(d.items);sTp(d.totalPages);sTt(d.total);}
    }catch{}finally{sF(false);}
  },[pg,se]);
  ...
  useEffect(()=>{
    ...
    ob.current=new IntersectionObserver((es)=>{
      es.forEach((e)=>{if(e.isIntersecting){const id=e.target.getAttribute('data-id');if(id)sVc((p)=>new Set(p).add(id));ob.current?.unobserve(e.target);}});
    },{threshold:0.1,rootMargin:'0px 0px -50px 0px'});
    cr.current.forEach((el)=>{if(ob.current)ob.current.observe(el);});
    return()=>ob.current?.disconnect();
  },[n]);
  ...
  const hi=async(e:React.ChangeEvent<HTMLInputElement>)=>{
    const fl=e.target.files;if(!fl||fl.length===0)return;sUp(true);
    try{for(let i=0;i<fl.length;i++){const f=fl[i];const fd=new FormData();fd.append('file',f);const r=await fetch('/api/notes/upload',{method:'POST',body:fd});const j=await r.json();if(j.success)sFi(p=>[...p,j.data]);}}
    catch{}finally{sUp(false);}if(fir.current)fir.current.value='';
  };

  const ri=(i:number)=>sFi(p=>p.filter((_,idx)=>idx!==i));
  const hsv=async(e:React.FormEvent)=>{
    e.preventDefault();if(!fc&&fi.length===0){alert('??????????');return;}sSv(true);
  ...
  if(!user)return(<div className="max-w-3xl mx-auto px-4 py-20 text-center animate-fadeIn">
    ...
    <a href="/login" className="inline-block px-6 py-2.5 bg-gradient-to-r from-yellow-400 to-pink-400 text-white rounded-xl text-sm font-medium hover:shadow-lg hover:scale-105 transition-all duration-300">??? ?</a>
  </div>);

  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-24 md:pb-6">
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-yellow-500 via-pink-500 to-purple-500 bg-clip-text text-transparent flex items-center gap-2">?? ???</h1>
      ...
      <form onSubmit={hs} className="mb-5">
        <div className="relative group">
          ...
          <input type="text" value={si} onChange={(e)=>sSi(e.target.value)} placeholder="?? ????..." className="w-full pl-9 pr-4 py-3 bg-white/80 backdrop-blur border-2 border-yellow-200 rounded-2xl text-sm focus:outline-none focus:border-pink-300 focus:ring-4 focus:ring-pink-100 transition-all duration-300" />
      ...
      {f?(<div className="flex justify-center py-20">
        <div className="flex flex-col items-center gap-3"><div className="w-10 h-10 border-[3px] border-yellow-300 border-t-pink-400 rounded-full animate-spin" /><p className="text-sm text-gray-400 animate-pulse">???...</p></div>
      </div>):n.length===0?(<div className="text-center py-20 animate-fadeIn">
        ...
        <p className="text-gray-400 mb-2">{se?'?? ?????????':'?? ?????'}</p>
        <p className="text-gray-300 text-sm mb-4">{se?'????????':'??????????'}</p>
        {!se&&<button onClick={oa} className="px-5 py-2 bg-gradient-to-r from-yellow-400 to-pink-400 text-white rounded-xl text-sm font-medium hover:shadow-lg hover:scale-105 transition-all duration-300">? ???????</button>}
      </div>):(
      <><div className="flex items-center justify-between mb-4"><p className="text-xs text-gray-400 flex items-center gap-1"><span className="inline-block w-2 h-2 bg-yellow-300 rounded-full animate-pulse" />? {tt} ?{se&&<span className="ml-1">? ??: {se}</span>}</p></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
        {n.map((note,idx)=>{
          const{color,rot}=getStyle(idx);
          const vis=vc.has(note._id);
          const hi=note.images&&note.images.length>0;
          const ih=hi&&note.images.length>=2;
          return(
            <div key={note._id} ref={(el)=>sr(note._id,el)} data-id={note._id}
              ...
              <div onClick={()=>oe(note)} className={[color.bg,'border-2',color.border,'rounded-2xl','overflow-hidden',color.shadow,'shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] group cursor-pointer'].join(' ')}>
                ...
                <div className="p-3.5 sm:p-4">
                  {note.title&&<h3 className="font-bold text-sm sm:text-base truncate flex items-center gap-1.5">
                    <span className={['inline-block w-2 h-2 rounded-full',color.dark.replace('text-','bg-')].join(' ')} />{note.title}
                  </h3>}
      ...
      {sm&&<div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-end sm:items-center justify-center animate-fadeIn" onClick={()=>sSm(false)}>
        <div className="bg-white w-full sm:max-w-lg sm:rounded-3xl rounded-t-3xl max-h-[85vh] overflow-y-auto shadow-2xl animate-slideUp" onClick={(e)=>e.stopPropagation()}>
          <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3.5 flex items-center justify-between z-10 rounded-t-3xl">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2"><span className="text-lg">{ei?'??':'??'}</span>{ei?'????':'????'}</h2>
          ...
          <form onSubmit={hsv} className="p-4 space-y-4">
            <div><label className="block text-xs font-medium text-gray-400 mb-1.5">?? ??</label><input type="text" value={ft} onChange={(e)=>sFt(e.target.value)} placeholder="???????????" className="w-full px-3 py-2.5 border-2 border-yellow-200 rounded-xl text-sm focus:outline-none focus:border-pink-300 focus:ring-4 focus:ring-pink-50 transition-all duration-300" /></div>
            <div><label className="block text-xs font-medium text-gray-400 mb-1.5">?? ??</label><textarea value={fc} onChange={(e)=>sFc(e.target.value)} placeholder="????..." rows={4} className="w-full px-3 py-2.5 border-2 border-yellow-200 rounded-xl text-sm focus:outline-none focus:border-pink-300 focus:ring-4 focus:ring-pink-50 transition-all duration-300 resize-none" /></div>
            <div>
        ...
        <div className="bg-white rounded-3xl p-6 mx-4 max-w-sm w-full shadow-2xl animate-scaleIn" onClick={(e)=>e.stopPropagation()}>
          <div className="text-center">
            ...
            <div className="flex gap-3">
              <button onClick={()=>sDi(null)} className="flex-1 py-2.5 border-2 border-gray-100 text-gray-600 font-medium rounded-xl hover:bg-gray-50 transition-colors text-sm">?????</button>
              <button onClick={()=>dn(di)} className="flex-1 py-2.5 bg-gradient-to-r from-red-400 to-rose-400 text-white font-medium rounded-xl hover:shadow-lg active:scale-[0.98] transition-all text-sm">????</button>
            </div>
          </div>
        </div>
      </div>}
      {li&&<div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center touch-none select-none animate-fadeIn" onClick={cl}>
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white/15 backdrop-blur rounded-full px-4 py-2 shadow-lg">
          ...
          <button onClick={(e)=>{e.stopPropagation();sZm(1);sPn({x:0,y:0});}} className="ml-2 px-3 py-1 text-xs text-white/70 hover:text-white bg-white/20 rounded-full hover:bg-white/40 transition-colors">??</button>
        </div>
